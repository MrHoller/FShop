...
use mrholler\shop\events\ShopOpenEvent;
use mrholler\shop\events\ShopPlayerAddItem;
use mrholler\shop\events\ShopPlayerBuyItem;
use mrholler\shop\events\ShopPlayerRemoveItem;
use mrholler\shop\libs\xenialdan\customui\windows\CustomForm;
use mrholler\shop\libs\xenialdan\customui\windows\ModalForm;
use mrholler\shop\libs\xenialdan\customui\windows\SimpleForm;
        ...
        $form->setCallable(function(Player $player, $data) use($msg){
            /*if($msg == 0) {
                if (isset($data[0]) and !empty($data[0]) and filter_var($data[0], FILTER_VALIDATE_INT) === false) {
                    ...
                    if($ev->isCancelled())
                        return false;
                    $shop[(string) $data[0]] = ["hide" => $data[1] ?? false];
                ...
                if (isset($data[1]) and !empty($data[1])) {
                    ...
                    if($this->shop->exists($data[1])){
                        $this->showAddCategory($player, 3);
                        return false;
                    }
                    $shop[(string) $data[1]] = ["hide" => $data[2] ?? false];
                    $this->shop->setAll($shop);
                    $this->reloadShop();
